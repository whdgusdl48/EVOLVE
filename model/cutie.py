...
from cutie.model.transformer.object_summarizer import ObjectSummarizer, EventSummarizer
...
class CUTIE(nn.Module):
    def __init__(self, cfg: DictConfig, *, single_object=False):
        ...
        self.evt_feat_proj = nn.Conv2d(self.ms_dims[0], self.pixel_dim, kernel_size=1)
        self.key_proj = EventKeyProjection(model_cfg)
    ...
    def encode_image(self, image: torch.Tensor, event: torch.Tensor) -> (Iterable[torch.Tensor], torch.Tensor):
        image = (image - self.pixel_mean) / self.pixel_std
        ms_feat_f = self.pixel_encoder(image)
        ms_feat_evt = self.event_encoder(event)

        ms_feat = []
        ms_feat.append(ms_feat_f[0])
        ms_feat.append(ms_feat_f[1])
        ms_feat.append(ms_feat_f[2])

        ms_e = []
        ms_e.append(ms_feat_evt[0])
        ms_e.append(ms_feat_evt[1])
        ms_e.append(ms_feat_evt[2])

        return ms_feat, ms_feat_evt, self.pix_feat_proj(ms_feat_f[0]), self.evt_feat_proj(ms_feat_evt[0])

    def encode_mask(
            self,
            image: torch.Tensor,
            ms_features: List[torch.Tensor],
            ms_events: List[torch.Tensor],
        ...
        if self.object_transformer_enabled:
            ms_events = ms_events.unsqueeze(1).repeat(1, mask_value.shape[1], 1, 1, 1)
            object_summaries, object_logits = self.object_summarizer(masks, mask_value,
                                                                     need_weights)
            object_summaries2, object_logits2 = self.event_summarizer(masks, ms_events + mask_value,
                                                                      need_weights)
        else:
            object_summaries, object_logits = None, None
        return mask_value, new_sensory, object_summaries, object_logits, object_summaries2

    def transform_key(self,
                      final_pix_feat: torch.Tensor,
                      final_evt_feat: torch.Tensor,
                      *,
                      need_sk: bool = True,
        ...
        key, shrinkage, selection = self.key_proj(final_pix_feat, need_s=need_sk,
                                                          need_e=need_ek)
        return key, shrinkage, selection

    def transform_event_key(self,
                            final_evt_feat: torch.Tensor,
                            *,
                            need_sk: bool = True,
                            need_ek: bool = True) -> (torch.Tensor, torch.Tensor, torch.Tensor):
        # key, shrinkage, selection = self.key_proj(final_pix_feat, final_evt_feat, need_s=need_sk, need_e=need_ek)
        key, shrinkage, selection = self.event_key_proj(final_evt_feat, need_s=need_sk, need_e=need_ek)
        return key, shrinkage, selection

    def mask_offset_conv(self, msk_value, offset, sh=None):
        if sh is not None:
            # inference
            b, c, hw = msk_value.shape
            b, o, hw = offset.shape
            msk_value = msk_value.view(b, c, sh[0], sh[1])
            offset = offset.view(b, o, sh[0], sh[1])
            msk_value = torchvision.ops.deform_conv2d(input=msk_value,
                                                      offset=offset,
                                                      weight=self.regular_conv.weight,
                                                      bias=self.regular_conv.bias,
                                                      padding=1,
                                                      )
            msk_value = msk_value.contiguous().flatten(start_dim=2)
            return msk_value
        else:
            b, n, c, t, h, w = msk_value.shape
            offset = offset.repeat(1, n, 1, 1, 1, 1)
            msk_value = msk_value.permute(0, 1, 3, 2, 4, 5)
            offset = offset.permute(0, 1, 3, 2, 4, 5)
            msk_value = msk_value.flatten(start_dim=0, end_dim=2)
            offset = offset.flatten(start_dim=0, end_dim=2)
            msk_value = torchvision.ops.deform_conv2d(input=msk_value,
                                                      offset=offset,
                                                      weight=self.regular_conv.weight,
                                                      bias=self.regular_conv.bias,
            ...
            msk_value = msk_value.contiguous().view(b, n, c, t, h, w)
    ...
    def read_memory(self, query_key: torch.Tensor, query_selection: torch.Tensor,
                    e_query_key: torch.Tensor, e_query_selection: torch.Tensor,
                    memory_key: torch.Tensor, memory_shrinkage: torch.Tensor,
                    e_memory_key: torch.Tensor, e_memory_shrinkage: torch.Tensor,
                    msk_value: torch.Tensor, obj_memory: torch.Tensor, e_obj_memory: torch.Tensor,
                    pix_feat: torch.Tensor,
                    event_feat: torch.Tensor, sensory: torch.Tensor, last_mask: torch.Tensor,
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            e_affinity = get_affinity(e_memory_key.float(), e_memory_shrinkage.float(), e_query_key.float(),
                                      e_query_selection.float())
            ...
            event_readout = readout(e_affinity, msk_value)
            event_readout = event_readout.view(batch_size, num_objects, self.value_dim,
                                               *pixel_readout.shape[-2:])
    ...
    def event_fusion(self,
        ...
        last_mask = F.interpolate(last_mask, size=sensory.shape[-2:], mode='area')
        last_others = self._get_others(last_mask)
        fused = self.event_fuser(pix_feat,
                                 pixel,
                                 sensory,
                                 last_mask,
                                 last_others,
