...
class VideoReader(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # froms = int(event[:-4])
        # if froms < 10:
        #     event = '0000' + str(froms) + '.npy'
        # elif froms >= 10 and froms < 100:
        #     event = '000' + str(froms) + '.npy'
        # elif froms >= 100:
        #     event = '00' + str(froms) + '.npy'
        
        # if int(frame[:-4]) == len(self.frames):
        #     frame = self.frames[-1]
        #     event = self.events[-1]
        frame = self.frames[idx]
        event = self.events[idx]
        ...
        event_path = path.join(self.event_dir, event)
        event = np.load(path.join(event_path))
        ...
        event = self.im_transform(event)
        if resize_needed:
            img = self.im_resize(img)
            event = sefl.im_resize(event)
