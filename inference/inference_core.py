...
class InferenceCore:
    ...
    def _add_memory(self,
        ...
        msk_value, sensory, obj_value, _, e_obj_value = self.network.encode_mask(
            image,
            pix_feat,
            event_feat,
    ...
    def _segment(self,
        ...
        memory_readout = self.memory.read(pix_feat, key, selection, e_key, e_selection, ms_event, self.last_mask, self.network)
    ...
    def step(self,
        ...
        if self.flip_aug:
            ...
            event = torch.cat([event, torch.flip(event, dims=[-1])], dim=0)
        ...
        # encoding the image
        ms_feat, ms_events, pix_feat, ms_event = self.image_feature_store.get_features(self.curr_ti, image, event)
        key, shrinkage, selection, offset = self.image_feature_store.get_key(self.curr_ti, image, event)
        e_key, e_shrinkage, e_selection = self.image_feature_store.get_event_key(self.curr_ti, image, event)
