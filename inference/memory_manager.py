...
class MemoryManager:
    ...
    def __init__(self, cfg: DictConfig, object_manager: ObjectManager):
        ...
        self.e_work_mem = KeyValueMemoryStore(save_selection=self.use_long_term,
                                              save_usage=self.use_long_term)
        if self.use_long_term:
    ...
    def _get_e_object_mem_by_ids(self, obj_ids: List[int]) -> torch.Tensor:
        if obj_ids[0] not in self.obj_e:
            # should only happen when the object transformer has been disabled
            return None
        return torch.stack([self.obj_e[obj] for obj in obj_ids], dim=1)
    ...
    def read(self, pix_feat: torch.Tensor, query_key: torch.Tensor, selection: torch.Tensor,
             e_query_key: torch.Tensor, e_selection: torch.Tensor, event: torch.Tensor,
        ...
        e_query_key = e_query_key.flatten(start_dim=2)  # bs*C^k*HW
        e_selection = e_selection.flatten(start_dim=2)  # bs*C^k*HW
        """
        ...
        for bucket_id, bucket in buckets.items():
            if self.use_long_term and self.long_mem.engaged(bucket_id):
                ...
                e_memory_key = self.e_work_mem.key[bucket_id]
                e_shrinkage = self.e_work_mem.shrinkage[bucket_id]
                similarity = get_similarity(memory_key, shrinkage, query_key, selection)
                e_similarity = get_similarity(e_memory_key, e_shrinkage, e_query_key, e_selection)
                ...
                if self.use_long_term:
                    ...
                    e_affinity = do_softmax(e_similarity, top_k=self.top_k, inplace=True)
            ...
            for objects in object_chunks:
                ...
                # this_msk_value += memory_key.unsqueeze(1).repeat(1, this_msk_value.shape[1], 1,1) # (1/2)*num_objects*C*N
                visual_readout = self._readout(affinity,
                                               this_msk_value).view(bs, len(objects), self.CV, h, w)
                visual_readout2 = self._readout(e_affinity,
                                                this_msk_value).view(bs, len(objects), self.CV, h, w)
                pixel_readout = network.pixel_fusion(pix_feat, visual_readout, this_sensory,
                                                     this_last_mask)

                # event_feat = event.unsqueeze(1).repeat(1, len(objects), 1, 1, 1)
                event_feat = network.event_fusion(event, visual_readout2, this_sensory, this_last_mask)
                ...
                this_e_obj_mem = self._get_e_object_mem_by_ids(objects)
                this_e_obj_mem = this_e_obj_mem.unsqueeze(2) if this_e_obj_mem is not None else None
                readout_memory, aux_features = network.readout_query(pixel_readout, event_feat, this_obj_mem,
                                                                     this_e_obj_mem)
    ...
    def add_memory(self,
                   ...
                   e_shrinkage: torch.Tensor,
                   msk_value: torch.Tensor,
                   obj_value: torch.Tensor,
                   e_obj_value: torch.Tensor,
                   objects: List[int],
                   selection: torch.Tensor = None,
                   e_selection: torch.Tensor = None,
                   *,
                   as_permanent: bool = False, network=CUTIE) -> None:
        # key: (1/2)*C*H*W
        ...
        # assert e_obj_value is None or e_obj_value.shape[0] == bs
        ...
        event_key = event_key.flatten(start_dim=2)

        shrinkage = shrinkage.flatten(start_dim=2)
        e_shrinkage = e_shrinkage.flatten(start_dim=2)
        offset = offset.flatten(start_dim=2)
        self.CK = key.shape[1]

        h, w = msk_value.shape[-2:]
        sh = [h, w]
        ...
        if selection is not None:
            ...
            e_selection = e_selection.flatten(start_dim=2)
        ...
        if e_obj_value is not None:
            for obj_id, obj in enumerate(objects):
                if obj in self.obj_e:
                    """streaming average
                    each self.obj_v[obj] is (1/2)*num_summaries*(embed_dim+1)
                    first embed_dim keeps track of the sum of embeddings
                    the last dim keeps the total count
                    ...
                    self.obj_v[obj] = torch.cat([self.obj_v[obj], obj_value[:, obj_id]], dim=0)
                    """
                    last_acc2 = self.obj_e[obj][:, :, -1]
                    new_acc2 = last_acc2 + e_obj_value[:, obj_id, :, -1]

                    self.obj_e[obj][:, :, :-1] = (self.obj_e[obj][:, :, :-1] +
                                                  e_obj_value[:, obj_id, :, :-1])
                    self.obj_e[obj][:, :, -1] = new_acc2
                else:
                    self.obj_e[obj] = e_obj_value[:, obj_id]

        # convert mask value tensor into a dict for insertion
        msk_values = {obj: network.mask_offset_conv(msk_value[:, obj_id], offset, sh) for obj_id, obj in
                      enumerate(objects)}
        ...
        for bucket_id in self.e_work_mem.buckets.keys():
            # long-term memory cleanup
            if self.use_long_term:
                # Do memory compressed if needed
                if self.e_work_mem.non_perm_size(bucket_id) >= self.max_work_tokens:
                    # Remove obsolete features if needed
                    if self.long_mem.non_perm_size(bucket_id) >= (self.max_long_tokens -
                                                                  self.num_prototypes):
                        self.long_mem.remove_obsolete_features(
                ...
                # FIFO
                self.e_work_mem.remove_old_memory(bucket_id, self.max_work_tokens)
